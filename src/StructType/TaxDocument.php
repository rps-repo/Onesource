...
class TaxDocument extends AbstractStructBase
{
    ...
    public function setDocumentNumber($documentNumber = null)
    {
        ...
        if ((is_scalar($documentNumber) && strlen($documentNumber) > 200) || (is_array($documentNumber) && count($documentNumber) > 200)) {
            throw new \InvalidArgumentException(sprintf('documentNumber: Invalid length, please provide an array with 200 element(s) or a scalar of 200 character(s) at most, "%d" length given', is_scalar($documentNumber) ? strlen($documentNumber) : count($documentNumber)), __LINE__);
        }
        // validation for constraint: minLength
        if ((is_scalar($documentNumber) && strlen($documentNumber) < 1) || (is_array($documentNumber) && count($documentNumber) < 1)) {
            throw new \InvalidArgumentException('documentNumber: Invalid length, please provide an array with 1 element(s) or a scalar of 1 character(s) at least', __LINE__);
        }
        // validation for constraint: string
        if (!is_null($documentNumber) && !is_string($documentNumber)) {
            throw new \InvalidArgumentException(sprintf('documentNumber: Invalid value, please provide a string, "%s" given', gettype($documentNumber)), __LINE__);
        }
